...
static void *slob_alloc(size_t size, gfp_t gfp, int align, int node)
{
	...
	list_for_each_entry(sp, slob_list, list) {
		...
		slob_acct(0, SLOB_UNITS(size) * SLOB_UNIT);
...
void *__kmalloc_node(size_t size, gfp_t gfp, int node)
{
	...
	if (size < PAGE_SIZE - align) {
		...
		m = slob_alloc(size + align, gfp, align, node);
 		cache_alloc_account(__builtin_return_address(0), m,
                                SLOB_ACCT(size + SLOB_UNIT), size);
...
void kfree(const void *block)
{
	...
	if (is_slob_page(sp)) {
		...
		cache_free_account(block, PAGE_ACCT(align) +
				SLOB_ACCT_S(*m+align));
		slob_free(m, *m + align);
	} else {
		cache_free_account((slob_t *)block-1, ksize(block));
		put_page(&sp->page);
	}
}
EXPORT_SYMBOL(kfree);
	...
	size_t align, unsigned long flags, void (*ctor)(void *))
{
	...
	cache_alloc_account(__builtin_return_address(0), c,
		SLOB_ACCT_S(kmem_cache_t), sizeof(kmem_cache_t));
...
void *kmem_cache_alloc_node(struct kmem_cache *c, gfp_t flags, int node)
{
	...
	if (c->size < PAGE_SIZE) {
		...
		cache_alloc_account(__builtin_return_address(0), b,
				SLOB_ACCT(c->size), c->size);
		...
		b = slob_new_pages(flags, get_order(c->size), node);
 		cache_alloc_account(__builtin_return_address(0), b,
 			PAGE_ACCT(get_order(c->size)), c->size);
