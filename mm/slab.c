...
module_param_named(max_order, slab_break_gfp_order, int, S_IRUSR);
...
kmem_cache_alloc_trace(size_t size, struct kmem_cache *cachep, gfp_t flags)
{
	...
	ret = __cache_alloc(cachep, flags, __builtin_return_address(0), -1);
					  ...
					  void *caller)
{
	...
	kmalloc_add_trace_entry(ret, size, flags, __builtin_return_address(0));
#endif
...
#ifdef CONFIG_KMALLOC_SIMPLE_TRACE
	kmalloc_del_trace_entry((void *)objp);
#endif
...
#endif
#ifdef CONFIG_DEBUG_SLAB_ACCOUNT
       proc_create("slab_account", S_IRUGO, NULL, &proc_slab_account_operations);
#endif
	return 0;
}
