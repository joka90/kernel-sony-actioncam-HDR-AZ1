...
ifeq ($(KBUILD_EXTMOD),)
core-y		+= kernel/ mm/ fs/ ipc/ security/ crypto/ block/ tools/ltt/
...
headers_install: __headers
	...
	@if [ ! -d "${INSTALL_USER_HDR_PATH}" ]; then \
		echo "ERROR: ${INSTALL_USER_HDR_PATH} does not exist. Please mkdir ${INSTALL_USER_HDR_PATH} or install userland."; \
	fi;
	@for dialectdir in $(shell find $(INSTALL_USER_HDR_PATH) -follow -maxdepth 1 -mindepth 1 -type d); \
	do \
		for dir in $$dialectdir/tool $$dialectdir/tool/devel; \
		do \
			dialect_exist=1; \
			if [ -L "$$dir/usr/include/asm" ]; then \
				echo "ERROR: 'make headers_install' does not support overwriting of $$dir/usr/include installed by old style 'make kernel_headers_install'. Please re-install userland to $$dir "; \
			else \
				$(srctree)/scripts/copy_headers.sh \
				${INSTALL_HDR_PATH}/include $$dir/usr/include;\
				find $$dir/usr/include -type d -print | \
					xargs chmod 755; \
				echo "Userland headers are installed to $$dir"; \
			fi; \
		done; \
	done; \
	if [ "$$dialect_exist" != 1 ]; then \
		echo "ERROR: ${INSTALL_USER_HDR_PATH}/DIALECT does not exist. Please mkdir ${INSTALL_USER_HDR_PATH}/DIALECT."; \
	fi;
...
MRPROPER_FILES += .config .config.old .version .old_version             \
		  ...
		  .target_name .arch_name .cross_compile		\
