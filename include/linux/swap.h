...
enum {
	...
	SWP_READONLY	= (1 <<16),	/* its a read only device */
};
...
extern int swap_set_ro(const char *specialfile, int ro);
...
static inline int swap_set_ro(const char *specialfile, int flag)
{
	return -EINVAL;
}
