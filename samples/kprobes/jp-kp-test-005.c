...
/*
 *  jp-kp-test-005.c - A Jprobe and a Kprobe with probe point to the kernel
 *  function 'sys_write'.
 *
...
/*
 * Jumper probe and kprobe for sys_write.
...
{
	printk("%s %d,kprobe pre-handler for sys_write \n", __FILE__, __LINE__);
...
{
	printk("%s %d,kprobe post-handler for sys_write \n", __FILE__, __LINE__);
	printk("%s %d,Stack dump : \n", __FILE__, __LINE__);
	dump_stack();
#if defined(CONFIG_ARM)
	printk("%s %d,The Registers are : \n", __FILE__, __LINE__);
...
static void __exit jprobe_kprobe_exit(void)
{
	...
	if (k_count1 > 0 && k_count2 > 0 && k_count3 > 0)
		printk("jp-kp-test-005 PASS");
	else
		printk("jp-kp-test-005 FAIL");
}
