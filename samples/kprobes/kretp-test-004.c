...
/*
 *  kretp-test-004.c - A Kretprobe with probe point to the kernel function 'sys_write'.
 *
...
static int ret_handler(struct kretprobe_instance *ri, struct pt_regs *regs)
{
	...
	if (retval < 0) {
		...
		printk("%s %d, Stack_dump :", __FILE__, __LINE__);
		dump_stack();
	}
	k_count++;
	return 0;
...
static void __exit kretprobe_exit(void)
{
	...
	if (k_count > 0)
		printk("kretp-test-004 PASS");
	else
		printk("kretp-test-004 FAIL");
}
