...
/*
 *  jp-kp-test-003.c - A Jprobe and a Kprobe with probe point to the kernel
...
/*
 * Jumper probe and kprobe for sys_gettimeofday.
...
{
	printk("%s %d,kprobe pre-handler for sys_gettimeofday\n", __FILE__, __LINE__);
...
{
	printk("%s %d,kprobe post-handler for sys_gettimeofday\n", __FILE__, __LINE__);
	printk("%s %d,Stack dump : \n", __FILE__, __LINE__);
	dump_stack();
#if defined(CONFIG_ARM)
	printk("%s %d,The Registers are : \n", __FILE__, __LINE__);
...
/* Proxy routine having the same arguments as actual sys_gettimeofday() routine */
long jsys_gettimeofday(struct timeval __user *tv, struct timezone __user *tz)
{
	printk("%s %d,jprobe: in sys_gettimeofday\n", __FILE__, __LINE__);
	printk("%s %d,Stack dump : \n", __FILE__, __LINE__);
	dump_stack();
...
static struct jprobe my_jprobe = {
	.entry = JPROBE_ENTRY(jsys_gettimeofday)
};

static int __init jprobe_kprobe_init(void)
{
	int ret;
	my_jprobe.kp.symbol_name = "sys_gettimeofday";
	...
	kp.symbol_name = "sys_gettimeofday";
...
static void __exit jprobe_kprobe_exit(void)
{
	...
	if (k_count1 > 0 && k_count2 > 0 && k_count3 > 0)
		printk("jp-kp-test-003 PASS");
	else
		printk("jp-kp-test-003 FAIL");
}
