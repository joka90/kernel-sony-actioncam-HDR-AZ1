...
/*
 *  jp-kp-test-002.c - A Jprobe and a Kprobe with probe point to the kernel
...
/*
 * Jumper probe and kprobe for do_gettimeofday.
...
{
	printk("%s %d,kprobe pre-handler for do_gettimeofday \n", __FILE__, __LINE__);
...
{
	printk("%s %d,kprobe post-handler for do_gettimeofday \n", __FILE__, __LINE__);
	printk("%s %d,Stack dump : \n", __FILE__, __LINE__);
	dump_stack();
#if defined(CONFIG_ARM)
	printk("%s %d,The Registers are : \n", __FILE__, __LINE__);
...
static void __exit jprobe_kprobe_exit(void)
{
	...
	printk("%s %d,jprobe unregistered for do_gettimeofday \n", __FILE__, __LINE__);
	unregister_kprobe(&kp);
	printk("%s %d,kprobe unregistered for do_gettimeofday \n", __FILE__, __LINE__);

	if (k_count1 > 0 && k_count2 > 0 && k_count3 > 0)
		printk("jp-kp-test-002 PASS");
	else
		printk("jp-kp-test-002 FAIL");
}
