...
static const char *sysmode_action[] = {
	"fixup",
	"fixup+warn",
};
...
static int alignment_proc_show(struct seq_file *m, void *v)
{
	...
	seq_printf(m, "System faults:\t%i (%s)\n", ai_sysmode,
			sysmode_action[ai_sysmode]);
	seq_printf(m, "Align fault:\t%s\n",
			cr_alignment & CR_A ? "enabled" : "disabled");
...
{
	char str[2];
	int mode, umode, smode;

	if (count > 0) {
		if (get_user(str[0], buffer))
			return -EFAULT;
		str[1] = '\0';

		/*
		 * cr_alignment will be set to control register at
		 * vector_swi or usr_entry.
		 */
		switch (str[0]) {
		case '+' :
			cr_alignment |= CR_A;
		...
		case '-' :
			...
			break;
		default :
			mode  = simple_strtoul(str, NULL, 16);
			umode = (mode >> 0) & 0x7;
			smode = (mode >> 3) & 0x1;

			if (umode >= 0 && umode <= 5)
				ai_usermode = safe_usermode(umode, true);
...
do_alignment(unsigned long addr, unsigned int fsr, struct pt_regs *regs)
{
	...
	if (thumb_mode(regs)) {
		...
		if(user_mode(regs)) {
			fs = get_fs();
			set_fs(KERNEL_DS);
			fault = __get_user(tinstr, (u16 __user *)ptr);
			set_fs(fs);
		} else {
		...
		if (!fault) {
			if (cpu_architecture() >= CPU_ARCH_ARMv7 &&
				...
				if(user_mode(regs)){
					fs = get_fs();
					set_fs(KERNEL_DS);
					fault = __get_user(tinst2, (u16 __user *)(ptr + 1));
					set_fs(fs);
				} else {
		...
		if(user_mode(regs)){
			fs = get_fs();
			set_fs(KERNEL_DS);
			fault = __get_user(instr, (u32 __user *)instrptr);
			set_fs(fs);
		} else {
	...
	if (ai_sysmode & SM_WARN)
		printk("Alignment trap (system): PC=0x%08lx Instr=0x%0*lx "
 ...
 user:
	...
	if (ai_usermode & UM_WARN)
		printk("Alignment trap (user): %s (%d) PC=0x%08lx Instr=0x%0*lx "
		       "Address=0x%08lx FSR 0x%03x\n", current->comm,
