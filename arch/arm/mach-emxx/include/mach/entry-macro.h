/* 2012-08-16: File added and changed by Sony Corporation */
/*
 * arch/arm/mach-emxx/include/mach/entry-macro.h
 *
 * Low-level IRQ helpers
 *
 * C code version of arch/arm/mach-emxx/include/mach/entry-macro.S,
...
#define IRQ_IS_EXTERNAL_INTERRUPT(irq)  ((irq) > 17)
#define IRQ_IS_IPI(irq)                 ((irq) < 16)
#define IRQ_IS_LOCALTIMER(irq)          ((irq) == 17)

#define get_irq_preamble() do {} while (0)

static inline unsigned int get_irqstat(void)
{
	const void __iomem __force *gic_base = __io_address(EMXX_INTA_CPU_BASE);
