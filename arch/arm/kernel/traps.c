...
#ifndef CONFIG_STACK_EXTENSIONS
...
#ifdef CONFIG_PREEMPT_RT_FULL
	/*
	 * If the regs are user mode then this check should not be
	 * actually necessary, But lets just be paranoid here.
	 */
	if (irqs_disabled() && interrupts_enabled(regs)){
		local_irq_enable();
		enable_irq = 1;
	}
#endif
...
#ifdef CONFIG_MACH_HAS_TRAP_INIT
	{
		extern void mach_trap_init(unsigned long);
		mach_trap_init(vectors);
	}
#endif
