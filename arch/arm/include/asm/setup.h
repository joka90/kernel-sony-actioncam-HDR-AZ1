...
/* SNSC NBLArgs */
#define ATAG_SNSC_NBLARGS	0x534e0002

struct tag_snsc_nblargs {
	__u32 len;
	__u32 addr;
	__u32 nbl_offset;
};
...
struct tag {
	...
	union {
		 ...
		 * SNSC NBLArgs
		 */
		struct tag_snsc_nblargs	snsc_nblargs;
	} u;
};
...
extern int __init arm_add_memory(phys_addr_t start, unsigned long size, unsigned long nid);
#else
