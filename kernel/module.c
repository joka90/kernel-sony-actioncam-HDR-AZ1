...
static inline int same_magic(const char *amagic, const char *bmagic,
			     bool has_crcs)
{
	return strncmp(amagic, bmagic,
		       CONFIG_SNSC_MODULE_VERMAGIC_LENGTH) == 0;
}
 ...
 */
static int __maybe_unused verify_export_symbols(struct module *mod)
{
	unsigned int i;
	struct module *owner;
...
#ifndef CONFIG_SNSC_SKIP_VERIFY_EXPORT_SYMBOLS
...
struct module *module_text_pfn(unsigned long pfn)
{
	struct module *mod;
	struct page *page;
	...
	list_for_each_entry(mod, &modules, list) {
		...
		end = start + mod->core_text_size;
		for (addr = start; addr < end; addr += PAGE_SIZE) {
			page = vmalloc_to_page((void *)addr);
			if (page && (pfn == page_to_pfn(page))) {
				preempt_enable();
				return mod;
			}
		}
