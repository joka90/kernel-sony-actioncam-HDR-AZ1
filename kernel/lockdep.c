...
DEFINE_PER_CPU(int, lock_stats_rt_trace);
#define LOCK_STATS_RT_TRACE per_cpu(lock_stats_rt_trace, raw_smp_processor_id())
#else
#define LOCK_STATS_RT_TRACE 1
#endif

#ifdef CONFIG_LOCK_STAT
int lock_stat = 1;
module_param(lock_stat, int, 0644);
int lock_stat_percpu;
int lock_stat_histogram;
#else
#define lock_stat 0
#define lock_stat_percpu 0
#define lock_stat_histogram 0
#endif
...
static inline void lock_time_add(struct lock_time *src, struct lock_time *dst)
{
	...
	{
		int bin;
		for (bin = 0; bin < MAX_LT_BIN; bin++)
			dst->bin[bin] += src->bin[bin];
	}
#endif
}

struct lock_class_stats *lock_class_stats_cpu(struct lock_class *class, int cpu)
{
	return &per_cpu(cpu_lock_stats, cpu)[class - lock_classes];
}
...
static void lock_release_holdtime(struct held_lock *hlock)
{
	...
	if (!lock_stat || !LOCK_STATS_RT_TRACE)
		return;
...
void lock_contended(struct lockdep_map *lock, unsigned long ip)
{
	...
	if (unlikely(!lock_stat || !LOCK_STATS_RT_TRACE))
		return;
