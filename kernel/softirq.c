...
/*
   - return 1 if pending softirq is only TIMER_SOFTIRQ.
     and the curr task is ksoftirqd.
   - else return 0.
 */
#ifdef CONFIG_SNSC_TIMER_SOFTIRQ_RQ_LOCK_GIVEUP
int softirq_is_timer(struct task_struct *curr)
{
...
DEFINE_TRACE(irq_tasklet_entry);
DEFINE_TRACE(irq_tasklet_exit);
...
static void wakeup_softirqd(void)
{
	...
	trace_off_mark_irq_dtl(0x2210, 0);
	if (tsk && tsk->state != TASK_RUNNING)
		wake_up_process(tsk);
	trace_off_mark_irq_dtl(0x221f, 0);
}
...
#ifdef CONFIG_EJ_SOFTIRQD_PRIORITY
static unsigned int sirqd_prio = 1;
module_param_named(sirqd, sirqd_prio, uint, S_IRUSR);
#endif

static inline void ksoftirqd_set_sched_params(void)
{
#ifdef CONFIG_SNSC_KSOFTIRQD_SCHED_OPTION
	struct sched_param param = { .sched_priority = CONFIG_SNSC_KSOFTIRQD_SCHED_PRIORITY };
	int policy = CONFIG_SNSC_KSOFTIRQD_SCHED_POLICY;
#else
#ifdef CONFIG_EJ_SOFTIRQD_PRIORITY
	struct sched_param param = { .sched_priority = sirqd_prio };
#else
...
#ifdef CONFIG_SNSC_KSOFTIRQD_SCHED_OPTION
        printk("%s(): setting scheduler policy=%d, prio=%d \n",
                       __FUNCTION__, CONFIG_SNSC_KSOFTIRQD_SCHED_POLICY, CONFIG_SNSC_KSOFTIRQD_SCHED_PRIORITY);
#endif
}
 ...
 */
inline void  _irq_exit(int call_invoke_softirq)
{
	account_system_vtime(current);
	trace_hardirq_exit();
	sub_preempt_count(IRQ_EXIT_OFFSET);
	if (call_invoke_softirq && !in_interrupt() && local_softirq_pending())
		invoke_softirq();
...
void irq_exit_no_invoke_softirq(void)
{
	_irq_exit(0);
}
...
inline void raise_softirq_irqoff(unsigned int nr)
{
	...
	trace_off_mark_irq_dtl(0x2120, nr);
	__raise_softirq_irqoff(nr);
	trace_off_mark_irq_dtl(0x212f, 0);
	 ...
	 */
	if (!in_interrupt()){
		trace_off_mark_irq_dtl(0x2130, 0);
		wakeup_softirqd();
		trace_off_mark_irq_dtl(0x213f, 0);
	}
...
void raise_softirq(unsigned int nr)
{
	...
	local_irq_save(flags);
	trace_off_mark_irq_dtl(0x2110, nr);
	raise_softirq_irqoff(nr);
	trace_off_mark_irq_dtl(0x211f, 0);
	local_irq_restore(flags);
}
...
again:
		trace_irq_tasklet_entry(t);
		t->func(t->data);
		trace_irq_tasklet_exit(t);
