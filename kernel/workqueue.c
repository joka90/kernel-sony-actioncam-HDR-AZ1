...
#ifdef CONFIG_SNSC_WORKQUEUE_SCHED_OPTION

#define WSCHD_PLCY    CONFIG_SNSC_WORKQUEUE_SCHED_POLICY
#define WSCHD_PRIO    CONFIG_SNSC_WORKQUEUE_SCHED_PRIORITY

if (set_thread_priority(WSCHD_PLCY, WSCHD_PRIO) < 0) {
	printk("%s(): setting scheduler policy=%d, prio=%d failed\n",
			       __FUNCTION__, WSCHD_PLCY, WSCHD_PRIO);
	}
#endif
