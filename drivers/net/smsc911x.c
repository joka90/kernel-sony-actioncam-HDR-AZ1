/* 2012-08-14: File changed by Sony Corporation */
/***************************************************************************
 *
...
static int smsc911x_stop(struct net_device *dev)
{
	...
	/* Reset PHY is required before software reset.
	 *   Software reset will be issue on next ndo_open.
	 */
	smsc911x_reg_write(pdata, PMT_CTRL, PMT_CTRL_PHY_RST_);
#endif
...
static int smsc911x_suspend(struct device *dev)
{
...
#ifndef CONFIG_ARCH_CXD90014BASED
	struct smsc911x_data *pdata = netdev_priv(ndev);
#ifdef CONFIG_SNSC_SSBOOT
	struct smsc911x_ctx *regs = &saved_regs;
#endif
	...
	if (!netif_running(ndev)) {
		printk(KERN_INFO "%s: netif not running\n", __FUNCTION__);
	return 0;
	}
	...
	regs->int_cfg   = smsc911x_reg_read(pdata, INT_CFG);
	regs->word_swap = smsc911x_reg_read(pdata, WORD_SWAP);
#endif
#else
	if (ndev) {
		if (netif_running(ndev)) {
...
static int smsc911x_resume(struct device *dev)
{
	...
	smsc911x_reg_write(pdata, INT_EN, 0);
	smsc911x_reg_write(pdata, INT_CFG, regs->int_cfg & ~INT_CFG_IRQ_EN_);

	/* restore register settings */
	smsc911x_reg_write(pdata, WORD_SWAP, regs->word_swap);
	...
	ret = smsc911x_phy_reset(pdata);
	if (ret < 0) {
		printk(KERN_CRIT
			"%s: smsc911x_phy_reset() failed.\n", __FUNCTION__);
		return -ENODEV;
	}
	ret = smsc911x_open(ndev);
	if (ret < 0) {
		printk(KERN_CRIT
			"%s: smsc911x_open() failed.\n", __FUNCTION__);
		return -ENODEV;
	}
