...
static void pci_setup_bridge(struct pci_bus *bus)
{
	...
	type = IORESOURCE_IO | IORESOURCE_MEM;
#endif /* CONFIG_ARCH_CXD90014BASED */
			...
			struct resource_list_x *add_head)
{
	...
	printk(KERN_DEBUG "PCI:%s:bus=%d:size=0x%x,add_size=0x%x,min_size=0x%x,min_align=0x%x\n"
	       , __func__, bus->number, size, add_size, min_size, min_align);
	if (!add_head) {
		if (size < add_size) {
			/* add_size is a space of hotplugguable slot. */
			size = add_size;
		}
	}
	...
	printk(KERN_DEBUG "PCI:%s:size0=0x%x,size1=0x%x\n", __func__, size0, size1);
	if (!size0 && !size1) {
		if (b_res->start || b_res->end)
			dev_info(&bus->self->dev, "disabling bridge window "
				 "%pR to [bus %02x-%02x] (unused)\n", b_res,
			...
			struct resource_list_x *add_head)
{
	...
	resource_size_t additional_pref_size = 0;
	...
	case PCI_CLASS_BRIDGE_PCI:
		...
		if (bus->self->is_hotplug_bridge) {
			...
			additional_pref_size = pci_hotplug_pref_size;
			if (ULONG_MAX == additional_pref_size)
				additional_pref_size = additional_mem_size;
		}
		/*
		 * Follow thru
		 */
	default:
		...
		if (pbus_size_mem(bus, prefmask, prefmask, 0, additional_pref_size, add_head))
			mask = prefmask; /* Success, size non-prefetch only. */
		else
			additional_mem_size += additional_pref_size;
