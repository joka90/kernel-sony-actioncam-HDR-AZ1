...
/* pci=hpprefsize=nnM can override this */
unsigned long pci_hotplug_pref_size = ULONG_MAX;
...
static int __init pci_setup(char *str)
{
	while (str) {
		...
		if (*str && (str = pcibios_setup(str)) && *str) {
			...
			} else if (!strncmp(str, "hpprefsize=", 11)) {
				pci_hotplug_pref_size = memparse(str+11, &str);
			} else {
