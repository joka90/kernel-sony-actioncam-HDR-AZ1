...
#if (defined(CONFIG_SCSI_SONY_EXT))
	scsi_sony_ext_event_connect_node(disk->major,minor);
#endif /* (defined(CONFIG_SCSI_SONY_EXT)) */
	return 0;
	...
	{	struct gendisk	*disk;
		if (cd) {
			...
			if (disk) {
				...
				/* NOTE: optical CD and DVD have no partition. 
				         Node number to access all media is major:first_minor.
				*/
				scsi_sony_ext_event_disconnect_node(disk->major,disk->first_minor);
			}
		}
	}
#endif /* (defined(CONFIG_SCSI_SONY_EXT)) */
