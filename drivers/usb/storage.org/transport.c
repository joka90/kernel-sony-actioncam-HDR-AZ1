...
static int usb_stor_msg_common(struct us_data *us, int timeout)
{
	...
	/* submit the URB */
#ifdef CONFIG_SNSC_USB_STORAGE_USE_GFP_NOWAIT_OR_ATOMIC
	status = usb_submit_urb(us->current_urb, GFP_NOWAIT);
	if (status)
		...
		unsigned int *act_len)
{
	...
	result = usb_sg_init(&us->current_sg, us->pusb_dev, pipe, 0,
			sg, num_sg, length, GFP_NOWAIT);
	if (result)
		result = usb_sg_init(&us->current_sg, us->pusb_dev, pipe, 0,
				sg, num_sg, length, GFP_ATOMIC);
#else
	result = usb_sg_init(&us->current_sg, us->pusb_dev, pipe, 0,
