...
static int _nbu2ss_enable_controller(struct nbu2ss_udc *udc)
{
	...
	writel(3, SMU_USBPHY_HOST_FUNC_SEL);
#endif
	...
	*/
	...
	_nbu2ss_bitset(&udc->p_regs->EPCTR, (DIRPD | PLL_RST | EPC_RST));
	udelay(2);

	_nbu2ss_bitclr(&udc->p_regs->EPCTR, DIRPD);
	mdelay(EPC_DIRPD_DISABLE_TIME);	/* 1ms wait */

	_nbu2ss_bitclr(&udc->p_regs->EPCTR, PLL_RST | EPC_RST);
#endif
 ...
 */
static int __devinit nbu2ss_drv_probe(struct platform_device *pdev)
{
	int	status = -ENODEV;
	struct nbu2ss_udc	*udc;
...
#ifdef USE_SUSPEND_WAIT
	udc->usb_suspend_workqueue =
		create_singlethread_workqueue("usb_suspend");
